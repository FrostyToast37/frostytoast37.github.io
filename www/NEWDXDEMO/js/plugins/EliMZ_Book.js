...
@plugindesc ♦5.4.7♦ Essential plugin for all Eli plugins.
...
Eli.Book = {
    ...
    initParameters(){
        ...
        this.parameters.engine = this.parseEngineParameters(parameters.engine)
        this.parameters.playtest = this.parsePlaytestParameters(parameters.playtest)
    },

    parseEngineParameters(rawParam){
    ...
    parsePlaytestParameters(rawParam){
        const param = JSON.parse(rawParam)
        return {
...
Graphics._setupPixi = function() {
    ...
    if(Utils.isOptionValid("test") && Eli.Book.parameters.playtest.startFps){
